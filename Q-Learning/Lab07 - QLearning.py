...
"""
@author: Blopa
Adapted by Josef Ruzicka B87905.

useful link: https://medium.com/analytics-vidhya/introduction-to-reinforcement-learning-q-learning-by-maze-solving-example-c34039019317
"""
...
### Default values
DEFAULT_ALPHA_LR = 0.1
DEFAULT_GAMMA_DISCOUNT = 0.1
DEFAULT_EPS_GREEDY = 1
DEFAULT_ETA_DECAY = 0.0001
...
class Agent():
    ...
    def __init__(self,seed,state_dims,actions,learning_rate,discount_factor,eps_greedy,decay):
        ...
        #qtable as tensor [row][col][up][right][left][down]
        self.qtable = []
        for row in range(state_dims[0]):
            current_row = []
            for col in range(state_dims[1]):
                cell = []
                for a in actions:
                    # note: 0:^, 1:>, 2:v, 3:<
                    cell.append(0)
                current_row.append(cell)
            self.qtable.append(current_row)
        #print(self.qtable)
        pass
    ...
    def step(self, env, learn=True):
        ...
        if (learn and (r_mode < self.eps_greedy)):
            action_value = self.prng.random() * 4
            #print('exploring')
            if ( action_value < 1):
                a = 0
            elif ( action_value < 2):
            ...
            # Q(s,a)       = self.qtable[env.position[0]][env.position[1]]
            # R(T(s,a))    = env.perform_action(0)[0]
            # max a'       = max(self.qtable[env.position[0]][env.position[1]]) after transitioning.
            # Q(T(s,a),a’) = env.perform_action(maxa)[0] after transitioning
            
            QSA  = self.qtable[env.position[0]][env.position[1]][a]
            ...
            self.qtable[env.position[0]][env.position[1]][a] +=\
            DEFAULT_ALPHA_LR *(\
            env.perform_action(a)[0] +\
            DEFAULT_GAMMA_DISCOUNT *\
            max(self.qtable[env.position[0]][env.position[1]]) -\
            QSA)
            
        # exploitation mode
        else:
            #print('exploitation')
            best_known_action = self.qtable[env.position[0]][env.position[1]].index(max(self.qtable[env.position[0]][env.position[1]]))
    ...
    # 4, con algunos map seeds como el 34301, no se encicla y logra llegar a la meta con facilidad.
    # a veces el programa no entra al if de exploracion aunque ambas condiciones requeridas sean true, 
    # no sé por qué, pero ese es mi problema principal
    # Sí logra determinar que los tesoros son valiosos y busca llegar a ellos si están más cercanos que la salida,
    # sin embargo, es difícil saber si busca otros por el problema del if de la exploración.
